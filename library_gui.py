...
def load_books_from_db():
    ...
    cur.execute("""
        SELECT book_id, title, author, active
        FROM books
        WHERE active = 1
        ORDER BY book_id;
    """)
    ...
    for book_id, title, author, active in rows:
        display_id = str(book_id)
...
def load_members_from_db():
    ...
    for member_id, first_name, last_name in rows:
        display_id = str(member_id)
...
def add_member():
    ...
    try:
        cur.execute("""
            INSERT INTO members (member_id, first_name, last_name, fines_due, active)
            VALUES (?, ?, ?, 0, 1);
...
def load_open_loans_from_db():
    ...
    cur.execute("""
        SELECT l.loan_id, b.book_id, m.member_id, l.due_date
        FROM loans l
    ...
    for loan_id, book_id, member_id, due_date in rows:
        loan_listbox.insert(tk.END, f"{loan_id} -> {member_id} (Due: {due_date})")
